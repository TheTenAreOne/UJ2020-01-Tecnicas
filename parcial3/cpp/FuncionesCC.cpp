...
void mostrarPersonasOrdenNombre( CentroC * centro ){
	...
	int piso, local, numPersonas;
	// AQUI SE USA EXCEPTION ------------ AQUI SE USA EXCEPTION -------------- AQUI SE USA EXCEPTION
	try{
		printf("Piso: "); scanf("%d", &piso);
		printf("Local: "); scanf("%d", &local);
		if( centro->locales[piso][local].disponible == DISPONIBLE ){
			throw std::logic_error ("Este local no ha sido arrendado\n");
		}
		numPersonas = centro->locales[piso][local].numPersonas;
		if( numPersonas == 0 ){
			throw std::logic_error ("Este local no tiene personas\n");
		}
